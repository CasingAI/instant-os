...
3. **触发**：宿主侧 ResizeObserver + debounce + 阈值。目标分辨率 = 宿主从标准档位表为 iframe **CSS 尺寸**选出的客机档（2026-08-27 定案：拉伸/等比最大化可见面积、原始下取；直发 CSS 值与纯客机覆盖式吸附各有硬伤，见 §8.9）。
...
- **`vgaMemoryMb` 的真实角色**：是**显存大小**，不是分辨率开关。帧缓冲字节 = 宽×高×bpp/8，必须 `≤ vgaMemoryMb`（软约束），再 `≤ 2560×1600`（硬约束）。设置档位 `[2,4,8,16,32,64,128,256]MB`（v86 硬上限 256MB）中，16MB 对应能装的屏 2560×1600×32（≈15.6MB，刚够生死线），更高档位留裕量。16MB 与 2560×1600×32 是 v86 默认设计成恰好握手的一对。
...
- ~~浏览器跨屏拖动导致的 DPR 变化不触发 ResizeObserver，需 `matchMedia('(resolution: ...)')` 补监听~~。**2026-08-27 作废**：目标分辨率以 iframe CSS 尺寸为测量基准（不乘 DPR），DPR 不再参与计算，matchMedia 监听与 devicePixelRatio 注入随之从宿主通道移除；同日晚些时候进一步演化为「宿主侧标准档位就近选档」，见 §8.9。
...
### 8.9 「直发 CSS 像素」的两个硬伤与宿主侧选档（2026-08-27 二次定案）

按用户决策去掉 DPR 后曾把目标定为 iframe CSS 尺寸 1:1 直发；运行时插桩日志（`.zcode/debug/20260827-res-align.log`，dpr=2 实测）证明两个硬伤：

- **低于 480 即沉默**：DPR=2 的 Retina 上面板 CSS 高度常只有 270–470（日志里 432×240 / 443×280 / 375×257…），clamp 返回 undefined，对齐器一次都不发——客机停在旧的大档位不动。
- **覆盖式吸附向上跳档**：即使发出（如 1096×618），客机「只选覆盖档」跳到 1152×864，高度多出 40%；「原始」显示模式（画布 1 客机px = 1 CSS px，超出视口即裁切滚动）把这些失配全部呈现为「画面被放大很多倍」。

**修复：选档上移到宿主**（`resolution-channel.ts selectResolutionMode`）。宿主持有标准档位表（640×480 … 2560×1600 共 17 档）：「拉伸/等比」**最大化实际可见面积**（等比缩放后画进视口的面积最大 = 黑边最少；同比例族并列取面积最接近视口的一档，比例匹配天然优先于大小）；「原始」走**下取**——只从两维都放得下视口的档位里取最大者（画布 1 客机px=1 CSS px 原样显示，超尺寸必被裁切滚动），连地板档都放不下的小视口回落地板。发出的就是客机会应用的精确值——05:20 轮 VGA 探针实锤发什么切什么、毫秒级闭环——客机代码零改动、无需重新拷贝 EXE；小视口自然落到 640×480 地板，「低于下限沉默」路径不复存在。
