...
| 栏 | 内容 |
|----|------|
| **已实现** | `path`、`buffer`、`events`、`assert`、`util`（薄：`inspect` / `inherits` / `promisify` / `types` / `format` / `deprecate` / `debuglog` / `parseArgs`）、`os`（薄：platform/arch/EOL/tmpdir/homedir/`version`/`userInfo` 等假值）、`perf_hooks`（薄：**宿主真实** `performance` 桥——`now` / `timeOrigin` / User Timing `mark`·`measure`·`clear*` / `getEntries*`；薄 `PerformanceObserver`；条目为 plain 对象；**无** ELU / `nodeTiming` / `timerify`）、`fs`、`fs/promises`（含 `realpath`/`realpathSync.native`、`copyFile`、`mkdtemp`、`truncate`、`readdir`+`withFileTypes`、`constants`、假 `chmod`/`chown`、`watch`/`watchFile`/`unwatchFile`、`createReadStream`/`createWriteStream`（分块推送；读侧仍整文件入宿主内存，受 `maxFileBytes`；**无** fd/`open`）；`module`（薄：`createRequire` 接 guest CJS；`enableCompileCache` 等 no-op；`isBuiltin`/`builtinModules` 反映已实现表；`Module` 未接）；`querystring`（`parse`/`stringify`/`encode`/`decode`）；`tty`（假：`isatty(0)=true`，`isatty(1/2)=false`，与 process stdio 策略对齐）；`console`（re-export 全局）；`timers`（转发全局 + 薄 `promises`）；`constants`（聚合 fs+os 常量）；`url`（`parse`/`format`/`fileURLToPath`/`pathToFileURL`；有引擎 `URL` 则 re-export）；`crypto`（薄：`randomBytes`/`randomUUID`；**宿主 @noble/hashes** 桥 `createHash`/`createHmac`——sha1/sha256/sha384/sha512/md5；**无** cipher/scrypt/sign）；`stream`（有限背压：`Readable`/`Writable`/`Duplex`/`Transform`/`PassThrough`；`pipe` pause/drain；最小 `finished`/`pipeline`；**无**对象模式全矩阵/fd 流）；`string_decoder`（薄：仅 UTF-8 `StringDecoder`）；`readline`（占位：`createInterface` 薄接口、`question` 异步空串；`readline/promises` 薄 `createInterface`+`question` Promise）；`zlib`（薄：**宿主 fflate** 同步 `*Sync` + **流式** `createGzip`/`createGunzip`/… Transform；**无** brotli）；`process` CLI 探测假值：`version` / `versions`（`node`/`v8`/`uv`/`openssl`/`modules` 静态假矩阵，**不**伪造 `electron`）/ `platform` / `arch` / `execPath` / stdout·stderr `isTTY: false` / `stdin.isTTY: true` / `argv` |
| **明确不做（本层）** | 完整 `builtinModules`（仅已实现子集）；`child_process`；`http`/`https`/`net`/`tls` 服务端；原生 addon；完整 Node `process.versions` 真值；`perf_hooks` 的 libuv 专有指标；完整 `crypto`（cipher/scrypt/sign 等） |
| **滚动中（撞墙再补）** | 非 UTF-8 `string_decoder`、交互式真 `readline`、fd/`fs.open`、超大文件不整文件入宿主的真流式读 |
