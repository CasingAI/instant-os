...
#   clipboard-bridge.exe        —— 由 scripts/build-clipboard-bridge.sh 产出（缺则现编）
...
CLIPBOARD_BRIDGE_BUILD="$ROOT/scripts/build-clipboard-bridge.sh"
...
# clipboard-bridge：out/ 里没有就现编一份。
if [ ! -f "$OUT_DIR/clipboard-bridge.exe" ]; then
  sh "$CLIPBOARD_BRIDGE_BUILD" "$OUT_DIR"
fi
...
# install-agent-v2.bat：全家桶安装脚本（agent 服务 + 信箱驱动 + 剪贴板桥
# 自启），与 exe/sys 一起拷进 XP 使用。服务方式是当前推荐安装形态。
