#!/usr/bin/env bash
# Download sherpa-onnx SenseVoice 五语识别 ONNX 权重到 public/assets/sense-voice。
...
#   - 用途：歌词对齐（中英日韩粤五语，CTC 识别，逐 token 时间戳），onnxruntime-web 直跑
...
DEST="$ROOT/public/assets/sense-voice"
TAG="asr-models"
REPO_DIR="sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2025-09-09"
...
EXPECTED_MODEL_BYTES="237115547"
...
# onnxruntime-web 不暴露模型 metadata，预提取 metadata_props（CMVN/LFR 参数）供 worker 读取
node "$ROOT/scripts/extract-onnx-metadata.mjs" "$DEST/models/model.int8.onnx" "$DEST/meta.json"
