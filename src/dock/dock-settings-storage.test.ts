...
{
  const settings = normalizeDockSettings({ sizeTier: 'large', desktopClickAction: 'flip3d' })
  assert.equal(settings.desktopClickAction, 'flip3d')
  assert.equal(resolveDesktopClickAction(settings), 'flip3d')
}

{
  const settings = normalizeDockSettings({ sizeTier: 'medium', desktopClickAction: 'nope' })
  assert.equal(settings.desktopClickAction, 'reveal')
}
