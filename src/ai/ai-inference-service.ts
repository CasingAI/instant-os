...
import SenseVoiceWorker from '../apps/align/sense-voice-worker.ts?worker'
...
export type AiModelId = 'stems-htdemucs' | 'stems-mdx' | 'align-zipformer' | 'align-sense-voice'
...
const AI_MODELS: Record<AiModelId, { label: string; createWorker: () => Worker }> = {
  ...
  'align-sense-voice': {
    label: 'SenseVoice 五语识别（歌词对齐）',
    createWorker: () => new SenseVoiceWorker(),
  },
}
