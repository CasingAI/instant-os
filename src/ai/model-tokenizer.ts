...
export type TokenizerFamily = 'deepseek-v4' | 'mimo' | 'mimo-v2-flash' | 'mimo-v2.5'
 ...
 */
export async function prepareTokenEstimation(model?: string): Promise<void> {
...
function getReadyTokenizer(model?: string): Tokenizer | undefined {
...
export function isModelTokenizerReady(model?: string): boolean {
  return getReadyTokenizer(model) !== undefined
}
...
export function countTokensWithModelTokenizer(
  ...
  const tokenizer = getReadyTokenizer(model)
  if (!tokenizer) {
    return undefined
  }
  try {
