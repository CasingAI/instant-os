...
import { listingSuggests3d } from './listing-tags.ts'
...
const EXISTING_3D_HTML_MARKERS = [
  /from\s+['"]three['"]/i,
...
function existingHtmlSuggests3d(html: string | undefined): boolean {
  const trimmed = html?.trim()
  if (!trimmed) {
    return false
  }

  return EXISTING_3D_HTML_MARKERS.some((pattern) => pattern.test(trimmed))
}
...
export function appGenerationSuggests3d(
...
export function resolveApp3dGenerationOptions(
  ...
  const is3d = appGenerationSuggests3d(listing, detail) || existingHtmlSuggests3d(existingHtml)
  if (!is3d) {
