...
/** 歌词对齐终端默认工作区（与 ProDude 一致：用户主目录） */
export const PHONEME_DEFAULT_WORKSPACE = '/user'

export type PhonemeTerminalHostApi = {
  ensureAiTerminal: (
...
type PhonemeTerminalHostProps = {
  workspaceFolder: string
  onApiChange: (api: PhonemeTerminalHostApi | null) => void
}

/**
 * 隐藏挂载 InstantREPL，供歌词对齐 Agent 调用；不向用户展示终端 UI。
 * 与 ProDude 的 ProdudeTerminalHost 同构：按 (kind, ownerChatId) 复用会话。
 */
export function PhonemeTerminalHost({ workspaceFolder, onApiChange }: PhonemeTerminalHostProps) {
  ...
  const workspaceRoot = workspaceFolder.trim() || PHONEME_DEFAULT_WORKSPACE
  ...
  const api = useMemo<PhonemeTerminalHostApi>(
    () => ({
      ensureAiTerminal,
      getAiTerminalHandle,
      getAiTerminalSnapshot,
  ...
  return (
    <div class="phoneme__terminal-host" aria-hidden="true">
      {sessions.map((session) => (
        <div key={`${session.id}:${workspaceRoot}`} class="phoneme__terminal-slot">
          <TerminalReplPanel
            workspaceRoot={workspaceRoot}
            handleRef={(handle) => bindHandle(session.id, handle)}
            className="phoneme__terminal-panel"
            welcomeLines={[]}
