...
/**
 * 英文词形变化后缀（ASR 弱读/演唱常把词形词读成原形，如 putting→put、running→run）。
 * 匹配规则先于 2 字母分支：we↔we've（前缀差 2）不应被 2 字母距离分支提前拒掉。
 */
const INFLECTION_SUFFIXES = ['ing', 'ed', 'es', 'er', 'est', 's'] as const

/**
 * 长词是否为短词的词形变化：long = short + 后缀，或 long = short + 双写尾字母 + 后缀
 * （putting = put + t + ing、stopped = stop + p + ed）。只认已知词形后缀，不放开裸长度，
 * 避免 wel↔welcome（差 4 非词形后缀）这类被误放行。
 */
function isInflectedForm(short: string, long: string): boolean {
  if (long.length <= short.length) return false
  for (const suffix of INFLECTION_SUFFIXES) {
    if (!long.endsWith(suffix)) continue
    const stem = long.slice(0, long.length - suffix.length)
    if (stem === short) return true
    // 双写尾字母：stem = short + 尾字母（stop→stopp→stopped）
    const last = short[short.length - 1]
    if (stem.length === short.length + 1 && stem.startsWith(short) && stem.endsWith(last)) {
      return true
    }
  }
  return false
...
/**
 * 英文识别口误：加权编辑距离 + 截断词尾前缀规则 + 词形后缀。
 *  - 归一化相同 → true
 *  - 长词以短词为前缀且差 ≤3（talki↔talking 截断词尾、we↔we've 缩写扩写）
 *  - 词形变化后缀（putting↔put、running↔run、stopped↔stop）
...
function latinPhoneticMatch(x: string, y: string): boolean {
  ...
  if (isInflectedForm(short, long)) return true
...
/**
 * 合并块允许的最大时间间隙（秒）：连续两个识别单元间隔过大不允许拼接成一个词
 * （防句首词 + 句尾词的巧合拼接，如前一句尾字 + 后一句首字拼出长词）。
 */
export const BLOCK_MAX_GAP_SEC = 0.3

/**
 * 合并匹配代价：连续两个识别单元拼成一个歌词词（识别拆词，如 wel+come → welcome、
 * speak+ing → speaking）。与 blockMatchCost（连读融合 1→2）方向相反。
 * 前提：两段时间贴合（间隙 ≤ BLOCK_MAX_GAP_SEC）、拼接后与歌词词的拉丁发音匹配。
 * 命中 0；否则 3（不匹配的硬连在代价上必败于跳过）。
 */
function mergedMatchCost(
  hypA: HypUnit,
  hypB: HypUnit,
  ref: G2pUnit,
  lineRange?: LineRange,
  ...
  const refNorm = normalizeForMatch(ref.text)
  // 只对拉丁词合并成立（中文/标点/乱码符号不拼接）
  if (!/^[a-z0-9]+$/u.test(joined) || !/^[a-z0-9]+$/u.test(refNorm)) return SUBSTITUTE_COST
  return latinPhoneticMatch(joined, refNorm) ? 0 : SUBSTITUTE_COST
}
 ...
 * 与连续两个识别段合并成一个歌词单元（识别拆词，块合并）。
 * mergedSecond：每 ref 的合并块第二段下标（非合并块 -1），供锚点/追踪确定块区间。
 */
function alignTextDp(
  hyp: HypUnit[],
  refUnits: G2pUnit[],
  lineRange?: LineRange,
): { refToHyp: Int32Array; mergedSecond: Int32Array } {
  ...
  const mergedSecond = new Int32Array(refUnits.length).fill(-1)
  if (n === 0 || m === 0) return { refToHyp, mergedSecond }
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 1; j <= m; j++) {
      ...
      // 连续两个识别单元合并成一个歌词单元（识别拆词，如 wel+come→welcome）
      if (i >= 2) {
        const merged =
          cost[idx(i - 2, j - 1)] +
          mergedMatchCost(hyp[i - 2], hyp[i - 1], refUnits[phonable[j - 1]], lineRange)
        if (merged < best) {
          best = merged
          bp = 4
        }
  ...
  let j = m
  while (i > 0 || j > 0) {
    const p = ptr[idx(i, j)] as 0 | 1 | 2 | 3 | 4
    if (p === 0) {
      ...
      // 合并：两个识别单元拼成一个歌词单元，ref 指向块首 hyp，记录第二段下标
      refToHyp[phonable[j - 1]] = i - 2
      mergedSecond[phonable[j - 1]] = i - 1
      i -= 2
      j -= 1
    } else {
      // 块分裂：两个歌词单元共享同一识别段
  ...
  return { refToHyp, mergedSecond }
}

/**
 * 单个歌词单元 → 识别段时间戳。块分裂时（两个连续 ref 共享同一 hyp 段），
 * 按块内字符长度比例切分段时长，避免两个词都拿到整段时间；
 * 合并块时（一个 ref 对应连续两个 hyp 段），词时间 = 两段拼接的整个区间。
 */
export function anchorSpanForUnit(
  ...
  hyp: HypUnit[],
  refUnits: G2pUnit[],
  mergedSecond?: Int32Array,
): { start: number; end: number } | undefined {
  const h = refToHyp[j]
  if (h < 0 || h >= hyp.length) return undefined
  const second = mergedSecond?.[j]
  if (second !== undefined && second >= 0 && second < hyp.length) {
    return { start: hyp[h].start, end: hyp[second].end }
  }
...
export function alignTextToUnits(
  ...
  const { refToHyp, mergedSecond } = alignTextDp(hyp, refUnits, lineRange)
  for (let j = 0; j < refUnits.length; j++) {
    const span = anchorSpanForUnit(j, refToHyp, hyp, refUnits, mergedSecond)
    if (span) {
...
export type DtwBacktrace = {
  ...
  hypCount: number
  /** 合并块（连续两识别段拼一个歌词词）的第二段下标；非合并块 -1 */
  mergedSecond: Int32Array
}
...
export function alignTextBacktrace(
  ...
  if (refUnits.length === 0 || segments.length === 0) {
    return {
      refToHyp: new Int32Array(refUnits.length).fill(-1),
      hypCount: 0,
      mergedSecond: new Int32Array(refUnits.length).fill(-1),
    }
  }
  const hyp = expandHypSegments(segments)
  if (hyp.length === 0) {
  ...
  const { refToHyp, mergedSecond } = alignTextDp(hyp, refUnits, lineRange)
  return { refToHyp, hypCount: hyp.length, mergedSecond }
}
 ...
 * mergedSecond 为 DP 记录的合并块第二段下标，已归属该 ref，不当作未匹配块。
 */
export function collectPositionAnchors(
  ...
  for (let u = 0; u < refToHyp.length; u++) {
    ...
    if (h >= 0 && h < hyp.length) {
      isHypMatched[h] = 1
      // 合并块：块内第二段已归属同一 ref，不能当作未匹配块配给后续词
      const second = mergedSecond?.[u]
      if (second !== undefined && second >= 0 && second < hyp.length) isHypMatched[second] = 1
    }
  }
...
/**
 * 识别块 → 归属歌词单元下标（追踪图用）。合并块（wel+come→welcome）把块内两个
 * 识别单元都映射到同一 ref，让拆词识别的两个块都显示「对上该词」而不是第二个显示未对上。
 */
export function buildHypToRef(
  refToHyp: Int32Array,
  hypCount: number,
  mergedSecond?: Int32Array,
): Map<number, number> {
  ...
  for (let u = 0; u < refToHyp.length; u++) {
    const h = refToHyp[u]
    if (h < 0 || h >= hypCount) continue
    map.set(h, u)
    const second = mergedSecond?.[u]
    if (second !== undefined && second >= 0 && second < hypCount) map.set(second, u)
  }
  return map
}
