    ...
    mode === 'ask'
      ...
      : mode === 'plan'
        ? [
            ...
            '用 run_in_terminal（只读）与 github_status / github_diff / github_log 调研；唯一写出口是 write_plan（写入工作区临时容器 /tmp/Workspace/{hash}/vscode/plans/*.md 并打开，不污染工作区 Git）。',            '需求不清时先问 1–2 个关键问题，再写计划。信息足够后必须调用 write_plan 落盘，不要只用聊天长文替代。',
