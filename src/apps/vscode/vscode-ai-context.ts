    ...
    mode === 'ask'
      ...
      : mode === 'plan'
        ? [
            ...
            '用 run_in_terminal（只读）调研，含 await instant.git.status/diff/log；唯一写出口是 write_plan（写入工作区临时容器 /tmp/Workspace/{hash}/vscode/plans/*.md 并打开，不污染工作区 Git）。',
  ...
  const envLines =
    mode === 'ask' || mode === 'plan'
      ? [
          ...
          mode === 'plan'
            ? '- Plan：run_in_terminal 只读调研（搜索用 instant.grep；GitHub 用 instant.git）；唯一落盘出口是 write_plan（/tmp/Workspace/{hash}/vscode/plans/*.md）；Todos 必须用 GFM `- [ ]` 复选框'
      ...
      : [
          ...
          '- Agent 有终端相关工具与 update_plan；读写与副作用主要走终端脚本（如 fs.readFileSync / fs.writeFileSync / fs.unlinkSync）或 instant.git.*；计划进度回写用 update_plan（勿用终端改 plans/*.md）',
