...
// 旧 subst 方案的同步脚本/清单保留路径已退役（SMB 方案替代），不再服务。
assert.equal((await request('GET', '/__sync_script')).status, 404)
assert.equal((await request('GET', '/__sync_manifest')).status, 404)
