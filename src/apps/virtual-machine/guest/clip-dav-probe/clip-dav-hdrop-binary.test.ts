/**
 * clip-dav-hdrop.exe 产物校验单测（dav_clipboard_paste 一期探针助手）。
 * 运行：node --experimental-strip-types src/apps/virtual-machine/guest/clip-dav-probe/clip-dav-hdrop-binary.test.ts
 *
 * 断言口径与 vmfile-spike-binary.test.ts 一致（同一 zig cc -nostdlib 管线）：
 *   PE32 i386 → GUI 子系统 → OS/Subsystem 版本 5.01 → 导入表只含
 *   kernel32/user32/ole32/shell32（shell32 是 CommandLineToArgvW）→
 *   体积 < 200KB，两次独立编译结构等价，关键字符串（日志路径/DropEffect
 *   格式名）在产物里。
...
const BUILD_SCRIPT = join(PROBE_DIR, '..', '..', '..', '..', '..', 'scripts', 'build-clip-dav-hdrop.sh')

/** 剪贴板（user32）+ 内存/文件（kernel32）+ OLE 剪贴板（ole32）
 *  + 命令行拆分（shell32），XP 裸机自带。 */
const IMPORT_DLL_WHITELIST = new Set(['kernel32', 'user32', 'ole32', 'shell32'])
const REQUIRED_IMPORTS = ['kernel32', 'user32', 'ole32', 'shell32']
...
function buildInto(directory: string): Buffer {
  ...
  assert.equal(
    result.status,
    0,
    `build-clip-dav-hdrop.sh 失败：\n${result.stdout ?? ''}\n${result.stderr ?? ''}`,
  )
  return readFileSync(join(directory, 'clip-dav-hdrop.exe'))
}
...
function buildAndAssert(directory: string): PeInfo {
  ...
  for (const marker of ['clip-dav-hdrop.log', 'Preferred DropEffect', 'clip-dav-hdrop build=']) {
    assert.ok(image.includes(Buffer.from(marker, 'ascii')), `产物缺少关键字符串 ${marker}（防呆 marker）`)
  }
  ...
  assert.notEqual(pe.entryRva, 0, '入口点不能为 0（-nostdlib 自定义入口 clipdav_entry）')
...
function main() {
  if (!hasZig()) {
    console.log('SKIP: 未安装 zig（brew install zig），clip-dav-hdrop 产物校验只在装了工具链的环境跑')
    return
  }
  const infoA = buildAndAssert(mkdtempSync(join(tmpdir(), 'clip-dav-hdrop-a-')))
  const infoB = buildAndAssert(mkdtempSync(join(tmpdir(), 'clip-dav-hdrop-b-')))
  ...
  console.log(`clip-dav-hdrop-binary.test.ts ok (${infoA.imports.join(', ')})`)
}
