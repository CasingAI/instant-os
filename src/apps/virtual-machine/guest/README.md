...
报告窗，含服务 Start 类型与每个实例的 `LogConf` 资源分配现状；服务被禁用
按失败计）。**回滚**：`ivm-agent.exe /audio-uninstall` 删掉自建设备实例、
禁用 ctlsb16 服务并写下回滚标记 `C:\Tools\audio-uninstalled.flag`——自愈
见标记就让路，不再自动恢复；显式重跑 `/audio-install` 会清标记并解禁服务。
首次部署（2026-08-30 之前）的版本在「无实例」场景会因日志缓冲溢出崩掉
agent（弹「遇到问题需要关闭」）——换新 exe 即可。
...
>
> 血泪教训之二（2026-08-31）：`/audio-uninstall` 禁用服务后，向导/BIOS
> 枚举出的绑定实例卸载不删，install 与自愈全被「已绑定」短路，服务禁用
...
则不出现）。设备详情里选目录（可选挂载的真实目录 `/mount/…`）、选盘符
（A–Z 全可选，默认 Z:；与客机已有盘符冲突时映射不出现）、按「连接到虚拟
