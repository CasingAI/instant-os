...
export const BUILTIN_APP_ABOUT: Record<string, BuiltinAppAbout> = {
  ...
  'files-op-progress': {
    version: '文件操作进度',
    paragraphs: [
      '文件复制、粘贴、删除、压缩等长操作会展开为一扇系统迷你窗口，显示进度与剩余时间。',
      '关闭该窗口即取消尚未完成的操作。窗口使用统一标题栏，可拖动、可参与 Flip 3D。',
    ],
  },
  'file-info': {
    version: '文件属性查看器',
  ...
  'disk-utility': {
    version: '磁盘与卷信息',
    paragraphs: [
      '磁盘工具并列展示数据空间（IndexedDB，含废纸篓）、本机挂载卷与磁盘镜像卷。',
      '对磁盘镜像会读取 MBR 分区表与 FAT 引导扇区，用整盘横条展示分区与未分配空间，并支持抹掉、分区与推出；镜像与虚拟机共享互斥锁时会拒绝改写。',
      '选中数据空间时可查看浏览器 storage.estimate 配额、系统空间上限与持久化状态。',
    ],
  },
  keychain: {
    version: '凭证与 API Key',
